...
def load_user(user_id):
    return None  # For now, we're just using anonymous users

# Configure Jinja2 to also look in _includes directory
app.jinja_loader = FileSystemLoader([
    os.path.join(app.root_path, 'templates'),
    os.path.join(app.root_path, '_includes')
])
...
def serve_image(filename):
    """Serve images with proper path handling."""
...
def parse_post_markdown(content):
    """Parse markdown content and extract metadata."""
    md = markdown.Markdown(extensions=['meta', 'fenced_code', 'tables'])
    html = md.convert(content)
    metadata = md.Meta
    
    # Convert metadata from lists to single values where appropriate
...
def admin_authors():
    """Admin authors page."""
    try:
        with open('_data/authors.json', 'r') as f:
            authors = json.load(f)
    except FileNotFoundError:
        authors = {}
...
def admin_settings():
    """Admin settings page."""
    try:
        with open('_data/settings.json', 'r') as f:
            settings = json.load(f)
    except FileNotFoundError:
        settings = {
            'blog_title': 'CLAN.com Blog',
            'blog_description': '',
            'posts_per_page': 10,
            'clan_api_key': '',
            'clan_api_endpoint': 'https://api.clan.com/v1'
        }
...
@app.route('/api/cache_size')
def get_cache_size():
    """Get the size of the cache directory."""
    try:
        total_size = 0
        cache_dir = os.path.join(app.root_path, '_cache')
        ...
        return jsonify({'success': True, 'size': size_str})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)})
